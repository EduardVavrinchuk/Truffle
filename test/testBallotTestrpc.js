...
contract('Ballot (testrpc network)', accounts => {
	...
	it("should have 3 correct candidate names", () =>{
		...
		return Ballot.deployed().then(instance =>{
			...
			assert.equal(firstName, "Name1", "Incorrect first candidate name in list");
			assert.equal(secondName, "Name2", "Incorrect second candidate name in list");
			assert.equal(thirdName, "Name3", "Incorrect third candidate name in list");
		})
	});
	...
	it("should display correct winner canidate", () => {
		...
		return Ballot.deployed().then(instance => {
			...
			assert.equal(winner, "Name1", "Incorrect winner's name");
		});
	});
});
