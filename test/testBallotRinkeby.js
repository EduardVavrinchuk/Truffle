...
contract("Ballot (rinkeby network)", accounts => {
	...
	it("should have 3 correct candidate names", () => {
		...
		return Ballot.deployed().then(instance => {
			...
			firstName = web3.toUtf8(proposal[0]);
			return ballot.proposals.call(1);
		}).then(proposal => {
			secondName = web3.toUtf8(proposal[0]);
			return ballot.proposals.call(2);
		}).then(proposal => {
			thirdName = web3.toUtf8(proposal[0]);
		
			assert.equal(firstName, "voter1", "Incorrect first candidate name in list");
			assert.equal(secondName, "voter2", "Incorrect second candidate name in list");
			assert.equal(thirdName, "voter3", "Incorrect third candidate name in list");
		});
	});
	...
	it("should display correct winner canidate", () => {
		...
		return Ballot.deployed().then(instance => {
			...
			assert.equal(winner, "voter1", "Incorrect winner's name");
		});
	});
